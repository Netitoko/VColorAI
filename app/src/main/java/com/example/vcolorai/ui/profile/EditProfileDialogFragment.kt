...
class EditProfileDialogFragment : DialogFragment() {
    ...
    companion object {
        ...
        fun newInstance(username: String, avatarUrl: String?): EditProfileDialogFragment {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val initialUsername = arguments?.getString(ARG_USERNAME).orEmpty()
        val initialAvatarUrl = arguments?.getString(ARG_AVATAR_URL)

        etUsername.setText(initialUsername)

        if (!initialAvatarUrl.isNullOrBlank()) {
            Glide.with(this).load(initialAvatarUrl).circleCrop().into(ivAvatar)
        } else {
        ...
        return AlertDialog.Builder(requireContext())
            ...
            .setPositiveButton("Сохранить", null) // перехватим, чтобы не закрывать при ошибке
            .create()
            .also { dlg ->
                dlg.setOnShowListener {
                    dlg.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                        val newUsername = etUsername.text.toString().trim()
                        saveProfile(newUsername, initialUsername, initialAvatarUrl)
                    }
                }
            }
    }
    ...
    private fun saveProfile(
        ...
        if (!isUsernameValid(newUsername)) {
            Toast.makeText(requireContext(), "Ник не должен содержать пробелы", Toast.LENGTH_SHORT).show()
            return
        }
        ...
        if (newAvatar == null) {
            ...
            // Ник (ключ) не менялся -> просто обновим username
            if (oldKey == newKey) {
                ...
                db.collection("users").document(uid)
                    ...
                    .addOnSuccessListener {
                        db.collection("public_users").document(uid)
                            .set(updates, SetOptions.merge())
                            .addOnSuccessListener {
                                sendResult(newUsername, initialAvatarUrl)
                                dismiss()
                            }
                            .addOnFailureListener { e ->
                                Toast.makeText(requireContext(), "Ошибка public профиля: ${e.message}", Toast.LENGTH_SHORT).show()
                            }
                    }
                    .addOnFailureListener { e ->
            ...
            // Ник меняется -> транзакция с usernames
        ...
        // ---------- 2) Аватар меняли: сначала грузим, потом обновляем ----------
        lifecycleScope.launch {
            try {
                val url = withContext(Dispatchers.IO) {
                    ImgBBUploader.uploadImage(requireContext(), newAvatar) // String?
                }

                if (url.isNullOrBlank()) {
                    Toast.makeText(requireContext(), "Не удалось загрузить аватар", Toast.LENGTH_SHORT).show()
                    return@launch
                }

                // Ник не менялся -> просто обновим username + avatarUrl
                if (oldKey == newKey) {
                    val updates = mapOf(
    ...
    // ✅ Транзакция: ВСЕ get() в начале, потом set/delete
    private fun runUsernameChangeTransaction(
        ...
        val newUsernameRef = db.collection("usernames").document(newKey)
        val oldUsernameRef = db.collection("usernames").document(oldKey)

        val userRef = db.collection("users").document(uid)
        val publicRef = db.collection("public_users").document(uid)
        ...
        db.runTransaction { tx ->
            ...
            // 2) WRITES
            tx.set(newUsernameRef, mapOf("uid" to uid, "createdAt" to now))
            ...
            val msg = when {
                ...
                else -> e.message ?: "Не удалось сохранить изменения"
            }
            Toast.makeText(requireContext(), msg, Toast.LENGTH_SHORT).show()
        }
    }

    // ✅ ВОТ ЕЁ НЕ ХВАТАЛО -> из-за этого и была ошибка компиляции
    private fun sendResult(username: String, avatarUrl: String?) {
